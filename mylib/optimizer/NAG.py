...
class NAG(BaseOptimizer):  # Nesterov Accelerated Gradient
    ...
    def optimize(self, layers: List[Layer], grad_W: np.ndarray, grad_b: np.ndarray):
        # ! пока что копия импульса
        v_W = [np.zeros_like(layer.W) for layer in layers]
        v_b = [np.zeros_like(layer.b) for layer in layers]
        for i in range(len(layers)):
            v_W[i] = self.beta * v_W[i] - self.learning_rate * grad_W[i]  # ! тут градиент в другой точке - как ее искать
